...
public class UserResource {
	...
	// Update User Profile
		...
		public ResponseEntity<CommonApiResponse> updateUserProfile(UpdateUserRequestDto updateUserRequestDto) {
		    LOG.info("Request received for updating user profile");

		    CommonApiResponse response = new CommonApiResponse();

		    if (updateUserRequestDto == null || updateUserRequestDto.getUserId() == 0) {
		        response.setResponseMessage("Invalid input data");
		        response.setSuccess(false);
		        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
		    }

		    User existingUser = userService.getUserById(updateUserRequestDto.getUserId());
		    ...
		    existingUser.setPhoneNo(updateUserRequestDto.getPhoneNumber());
		    ...
		    if (updatedUser == null) {
		        throw new UserSaveFailedException("Failed to update user profile due to technical issue");
		    }
